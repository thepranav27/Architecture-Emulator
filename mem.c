...
const uint64_t WRITE_CHAR_ADDR = 0xFFFFFFFFFFFFFFFFUL;
...
static bool is_special_addr(const uint64_t addr) {
    return ((addr == NULL_ADDR) || 
            (addr == WRITE_CHAR_ADDR) || 
            (addr == RET_FROM_MAIN_ADDR));
}
...
static uint64_t _mem_read_special(const uint64_t addr, const unsigned width) {
    if (NULL_ADDR == addr) {MISSING(); return 0;}
    if (WRITE_CHAR_ADDR == addr) {MISSING(); return 0;}
...
static write_ret_code_t _mem_write_special(const uint64_t addr, const uint64_t data, const unsigned width) {
    if (NULL_ADDR == addr) {MISSING(); return WRITE_SUCCESS;}
    if (WRITE_CHAR_ADDR == addr) {
        switch (width) {
